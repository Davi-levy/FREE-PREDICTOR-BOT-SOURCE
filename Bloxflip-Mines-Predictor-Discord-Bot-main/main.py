...
async def mines(ctx, round_id):
    ...
    if round_length < 36:
        await ctx.send("Milky has encountered a invalid id, try using another one!")
        ...
        em.set_footer(text="MILK PREDICTOR ⭐ | Coded by Levy")
        em.add_field(name="Mines predictor",value=row1 + "\n" + row2 + "\n" + row3 + "\n" + row4 +"\n" + row5 + "\n" + "**Accuracy**" + "\n" + info +"%")
        await ctx.reply(embed=em)


bot.run("MTAyODkzMzg2MDUzMDA2OTU0NA.GHp6Eb.sUf_zXBlGaJ6-WQPtRz_4b2uZ9-tqUL_o5N4Tg")